...
export const getDangerColor = (colorValue: string) => {
	return Color(colorValue).isLight() ? '#DE4242' : '#DE4242';
};
...
export const setGlobalColors = (primary: string, secondary: string) => {
	...
	if (regex.test(primary) && regex.test(secondary)) {
		...
		rootStyle.setProperty('--danger', Color(getDangerColor(secondary)).array().join(','));
		rootStyle.setProperty(
			'--contrast-danger',
			Color(getContrastColor(getDangerColor(secondary)))
				.array()
				.join(',')
		);
