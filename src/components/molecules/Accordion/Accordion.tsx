...
const Accordion: React.FC<AccordionProps> = ({ children, icon, isOpen, title }) => {
	...
	return (
		<div>
			<div className="flex items-center py-sm" role="button" onClick={handleClick}>
				<div className="w-48">{icon}</div>
				<div className="flex-grow text-contrast-secondary text-opacity-96 text-subtitle">
					{title}
				</div>
			...
			<div
				className={classNames('overflow-hidden duration-300', !isOpen && 'h-0')}
				ref={accordionBodyRef}
			>
				<div className="py-sm desktop:pl-48">{children}</div>
			</div>
		</div>
	);
};
